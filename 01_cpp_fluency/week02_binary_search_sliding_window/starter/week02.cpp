...
namespace cpp_course::week02
{
  ...
  std::ptrdiff_t rotated_search(const std::vector<int> &values, int target)
  {
    ...
    while (left <= right)
    {
      ...
      {
        if (values[left] <= target && values[mid] > target)
          right = mid - 1;
        else
          left = mid + 1;
      }
      ...
      {
        if (values[mid] < target && values[right] >= target)
          left = mid + 1;
        else
          right = mid - 1;
      }
  ...
  {
    if (piles.empty() || hours < static_cast<long long>(piles.size()))
      throw std::invalid_argument("NO BANANA TIME!");

    int maximum;
    int minimum = 1;

    // scoped so min/max are freed
    {
      auto [min, max] = std::minmax_element(piles.begin(), piles.end());
      if (*min < 1)
        throw std::invalid_argument("NO BANANS!");
      maximum = *max;
    }
  ...
  std::size_t minimum_window_length(const std::vector<int> &values, long long target)
  {
    for (const int &x : values)
      if (x < 0)
        throw std::invalid_argument("Negative value in 'values'.");
  ...
  std::size_t longest_unique_span(std::string_view text)
  {
    ...
    std::size_t answer = 0, begin = 0;
    std::array<size_t, 256> char_map;
    char_map.fill(std::string_view::npos);

    for (size_t i = 0; i < text.size(); i++)
    {
      if (char_map[text[i]] == std::string_view::npos || char_map[text[i]] < begin)
        char_map[text[i]] = i;

      else
      {
        begin = char_map[text[i]] + 1;
        char_map[text[i]] = i;
      }
